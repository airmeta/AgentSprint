...
public interface IAgileMvpService
{
    ...
    /// <summary>
    /// zh-cn: 创建可在项目和需求中选择的 Skill 配置，编码为空时自动生成且全局唯一，内容用于后续任务提示词上下文。
    /// en-us: Creates a selectable skill configuration for projects and requirements; when the code is empty, a globally unique code is generated for later task-prompt context.
    /// </summary>
    /// <param name="request">
    /// zh-cn: Skill 编码、名称、类型、说明和指令内容；编码为空时自动生成，类型为空时使用开发类型。
    /// en-us: Skill code, name, type, description, and instruction content; the code is generated when empty, and the development type is used when the type is empty.
    /// </param>
    /// <param name="userId">
    ...
    /// </returns>
    Task<IReadOnlyList<SprintSkillResult>> ListSkillsAsync(
    ...
    /// <summary>
    /// zh-cn: 更新 Skill 名称、类型、说明、内容和状态；停用后的 Skill 不再允许被新项目或新需求选择。
    /// en-us: Updates skill name, type, description, content, and status; disabled skills can no longer be selected by new projects or requirements.
    /// </summary>
    /// <param name="id">
    /// zh-cn: Skill 标识。
    ...
    /// <summary>
    /// zh-cn: 删除未被需求引用的功能模块，避免项目端下出现无效模块；已有需求绑定时会拒绝删除以保护需求归属关系。
    /// en-us: Deletes a feature module only when no requirement references it, preserving requirement ownership when the module is already in use.
    /// </summary>
    /// <param name="id">
    /// zh-cn: 功能模块标识。
    ...
    /// </returns>
    Task<bool> DeleteFeatureModuleAsync(string id);
    ...
    /// </returns>
    Task<IReadOnlyList<SprintRequirementResult>> ListRequirementsAsync(
        string? projectId,
        string? keyword = null,
        string? status = null,
        string? health = null);
    ...
    /// </returns>
    Task<IReadOnlyList<SprintRequirementReviewItemResult>> ListMyPendingReviewsAsync(
        string reviewerId,
