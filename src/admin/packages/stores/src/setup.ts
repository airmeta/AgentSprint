...
type PersistentStateStorage = Pick<
  Storage,
...
function createMemoryStorage(): PersistentStateStorage {
...
function createDevStorage(): PersistentStateStorage {
  try {
    const testKey = '__vben_store_probe__';
    localStorage.setItem(testKey, testKey);
    localStorage.removeItem(testKey);
    return localStorage;
  } catch (error) {
    console.warn(
      'localStorage is not accessible, falling back to memory storage:',
      error,
    );
