...
describe('useAccessStore', () => {
  ...
  it('does not add deprecated sprint tabs', () => {
    const store = useTabbarStore();
    const tab: any = {
      fullPath: '/sprint/mvp',
      meta: { title: 'MVP 工作台' },
  ...
  it('hides deprecated sprint tabs restored from persisted state', () => {
    const store = useTabbarStore();
    const staleTab: any = {
      fullPath: '/sprint/mvp',
      meta: { title: 'MVP 工作台' },
    ...
    const activeTab: any = {
      fullPath: '/sprint/tasks',
      meta: { title: '任务大厅' },
      name: 'SprintTasks',
      path: '/sprint/tasks',
      key: '/sprint/tasks',
    };
    store.tabs.push(staleTab, activeTab);

    expect(store.getTabs).toEqual([activeTab]);
  });
  ...
  it('uses the route full path as the keep-alive cache key', async () => {
    const store = useTabbarStore();
    ...
    expect(store.getCachedTabs).toEqual([
      'RouteCache__system_users',
      'RouteCache__system_roles',
    ]);
  });
