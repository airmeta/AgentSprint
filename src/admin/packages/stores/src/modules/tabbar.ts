...
const deprecatedSprintTabNames = new Set([
  'SprintMvp',
  'SprintTaskDecomposition',
]);

const deprecatedSprintTabPaths = new Set([
  '/sprint/mvp',
...
export const useTabbarStore = defineStore('core-tabbar', {
  actions: {
    ...
    addTab(routeTab: TabDefinition): TabDefinition {
      ...
      if (!isTabShown(tab) || isDeprecatedSprintTab(tab)) {
        return tab;
      }
    ...
    async refresh(router: Router | string) {
      ...
      const { currentRoute } = router;
      const currentKey = getTabCacheKey(currentRoute.value);

      this.excludeCachedTabs.add(currentKey);
      this.renderRouteView = false;
      startProgress();

      await new Promise((resolve) => setTimeout(resolve, 200));

      this.excludeCachedTabs.delete(currentKey);
    ...
    async refreshByName(name: string) {
      ...
      const key = tab ? getTabCacheKeyFromTab(tab) : name;
      this.excludeCachedTabs.add(key);
      await new Promise((resolve) => setTimeout(resolve, 200));
      this.excludeCachedTabs.delete(key);
    },
    ...
    async updateCacheTabs() {
      ...
      for (const tab of this.tabs) {
        ...
        cacheMap.add(getTabCacheKeyFromTab(tab));
      }
  ...
  getters: {
    ...
    getTabs(): TabDefinition[] {
      const normalTabs = this.tabs.filter(
        (tab) => !isAffixTab(tab) && !isDeprecatedSprintTab(tab),
      );
      return [...this.affixTabs, ...normalTabs]
        .filter(Boolean)
        .filter((tab) => !isDeprecatedSprintTab(tab));
    },
...
function isDeprecatedSprintTab(tab: TabDefinition) {
  const tabKey = getTabKeyFromTab(tab);
  const tabName = tab.name?.toString() ?? '';
  const path = tab.path ?? '';
  const fullPath = tab.fullPath ?? '';
  return (
    deprecatedSprintTabNames.has(tabName) ||
    deprecatedSprintTabPaths.has(path) ||
    deprecatedSprintTabPaths.has(fullPath) ||
    deprecatedSprintTabPaths.has(tabKey) ||
    [...deprecatedSprintTabPaths].some((deprecatedPath) =>
      tabKey.startsWith(`${deprecatedPath}?`),
    )
  );
}
...
function normalizeCacheComponentName(key: string) {
  return `RouteCache_${key.replaceAll(/[^A-Za-z0-9_]/g, '_')}`;
}

function getTabCacheKey(tab: RouteLocationNormalized | RouteRecordNormalized) {
  return normalizeCacheComponentName(getTabKey(tab));
}

function getTabCacheKeyFromTab(tab: TabDefinition): string {
  return normalizeCacheComponentName(getTabKeyFromTab(tab));
}
...
export { getTabCacheKey, getTabKey };
